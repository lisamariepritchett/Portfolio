...
%            i.e., experiment 1, bootstrapped estimate of width of guessing region
...
% find proportion of trials in each bin where observer gave response 2
presp = kresp./nresp;
...
errfn = @( p ) -sum(log( binopdf( kresp, nresp, fitfn( dlist, p ) ) ));

% fit the normal cdf; take best of several fits with random starting points
errmin = Inf;
for i = 1:20
    [ param, err ] = fminsearch( errfn, [ -0.25+0.5*rand 0.1+0.3*rand ] );
    if err<errmin
        param_norm = param;
        errmin = err;
    end
...
plot(dlist(nresp>=20),presp(nresp>=20),'ro'); hold off;
...
% pair from the decision line in the line above.
...
for b = 1:B+1
    ...
    if b == 1
        ...
        % on subsequent passes, resample the data from the binomial distribution to get bootstrapped estimates
        kstar = binornd( nresp, presp );
    end
    
    % make a maximum likelihood objective function
    errfn = @( p ) -sum(log( binopdf( kstar, nresp, fitfn( dlist, p ) ) ));
